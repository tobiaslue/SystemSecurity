...
static sgx_status_t SGX_CDECL sgx_computeSharedKey(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	sgx_ec256_public_t* _tmp_p_public_A = ms->ms_p_public_A;
	size_t _len_p_public_A = 64;
	sgx_ec256_public_t* _in_p_public_A = NULL;

	CHECK_UNIQUE_POINTER(_tmp_p_public_A, _len_p_public_A);
	...
	if (_tmp_p_public_A != NULL && _len_p_public_A != 0) {
		_in_p_public_A = (sgx_ec256_public_t*)malloc(_len_p_public_A);
		if (_in_p_public_A == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_p_public_A, _len_p_public_A, _tmp_p_public_A, _len_p_public_A)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	ms->ms_retval = computeSharedKey(_in_p_public_A);

err:
	if (_in_p_public_A) free(_in_p_public_A);
	return status;
}
...
static sgx_status_t SGX_CDECL sgx_completeChallenge(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_completeChallenge_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_completeChallenge_t* ms = SGX_CAST(ms_completeChallenge_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	unsigned char* _tmp_message = ms->ms_message;
	...
	ms->ms_retval = completeChallenge(_in_message, _in_result);
	if (_in_result) {
...
SGX_EXTERNC const struct {
	...
	6,
	{
		...
		{(void*)(uintptr_t)sgx_completeChallenge, 0, 0},
	}
};
